...
export default function BookingPage() {
  ...
  // ⭐ Load available time slots (availability + booked + blocked)
  async function loadAvailableTimes(selectedDate) {
    ...
    // 1️⃣ Load barber availability for that day
  ...
  async function createAppointment() {
    ...
    const { error } = await supabase.from("appointments").insert({
      business_id: barber.business_id || barber.business,
      barber_id: barberId,
      service,
      date,
      time: formattedTime,
